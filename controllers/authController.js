...
exports.signup = catchAsync(async (req, res, next) => {
  const { name, email, password, passwordConfirm, role } = req.body; //Do not send req.body as it is as not to include unnecessary details like role=admin by a user trying to be a admin
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // 1) Check if email and password exist in the input
  if (!email || !password) {
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!currentUser) {
    return next(new AppError('The user for the token no longer exists', 401));
  }
